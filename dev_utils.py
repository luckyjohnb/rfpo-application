...
def list_users():
    ...
    for user in users:
        ...
        print(f"{user['username']:<15} {user['email']:<25} {user['role']:<15} {user['status']:<10} {created_date:<20}")
...
def check_system_health():
    ...
    if locked_users:
        ...
        for user in locked_users:
            print(f"  - {user['username']} (locked until {user['account_locked_until']})")
    else:
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Development utility for Flask application')
