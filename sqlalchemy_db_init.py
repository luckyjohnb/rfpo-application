...
def create_admin_user(app, force_recreate=False):
    ...
    with app.app_context():
        ...
        if existing_admin:
            if force_recreate:
                print(
                    "🔄 Deleting existing admin user to recreate with correct password hash..."
                )
    ...
    try:
        with app.app_context():
            ...
            if missing_tables:
                ...
                print(
                    f"✅ All {len(expected_tables)} expected tables created successfully"
                )
    ...
    with app.app_context():
        ...
        for list_type, items in reference_data_sets:
            for key, value in items:
                ...
                existing_item = List.query.filter_by(type=list_type, key=key).first()
