...
class Consortium(db.Model):
    ...
    consort_id = db.Column(
        db.String(32), unique=True, nullable=False
    )  # External consortium ID (e.g., "00000008")
    ...
    terms_pdf = db.Column(
        db.String(255)
    )  # terms_pdf (terms and conditions PDF filename)

    # Configuration flags
    require_approved_vendors = db.Column(
        db.Boolean, default=True
    )  # consort_rfpo_approvedvendors (1=Yes, 0=No)
    non_government_project_id = db.Column(
        db.String(32)
    )  # consort_non_projects (project ID from dropdown)

    # User permissions (stored as JSON arrays for multi-select fields)
    rfpo_viewer_user_ids = db.Column(
        db.Text
    )  # consort_rfpo_viewers[] - JSON array of user IDs
    rfpo_admin_user_ids = db.Column(
        db.Text
    )  # consort_rfpo_admin[] - JSON array of user IDs
...
class RFPO(db.Model):
    ...
    files = db.relationship(
        "UploadedFile", backref="rfpo", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def update_totals(self) -> None:
        ...
        if self.line_items:
            self.subtotal = sum(
                float(item.total_price or 0) for item in self.line_items
            )
        else:
            self.subtotal = 0.00
    ...
    def to_dict(self):
        return {
            ...
            "cost_share_amount": (
                float(self.cost_share_amount) if self.cost_share_amount else 0.00
            ),
...
class RFPOLineItem(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            "capital_acquisition_date": (
                self.capital_acquisition_date.isoformat()
                if self.capital_acquisition_date
                else None
            ),
            "capital_condition": self.capital_condition,
            "capital_acquisition_cost": (
                float(self.capital_acquisition_cost)
                if self.capital_acquisition_cost
                else None
            ),
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "updated_at": self.updated_at.isoformat() if self.updated_at else None,
...
class Team(db.Model):
    ...
    record_id = db.Column(
        db.String(32), unique=True, nullable=False
    )  # External team ID (e.g., "00000098")
    name = db.Column(
        db.String(255), nullable=False
    )  # team_name (increased to match form maxlength)
    ...
    consortium_consort_id = db.Column(
        db.String(32), nullable=True
    )  # team_consort (consortium's consort_id)

    # Team-level user permissions (stored as JSON arrays)
    rfpo_viewer_user_ids = db.Column(
        db.Text
    )  # team_rfpo_viewers[] - JSON array of user IDs
    rfpo_admin_user_ids = db.Column(
        db.Text
    )  # team_rfpo_admin[] - JSON array of user IDs
    ...
    def __repr__(self):
        consortium_info = (
            f" (Consortium: {self.consortium_consort_id})"
            if self.consortium_consort_id
            else " (No Consortium)"
        )
...
class User(UserMixin, db.Model):
    ...
    company_code = db.Column(
        db.String(10)
    )  # BP, CHEV, DOE, EM, FCA, FRD, GM, Lab, P66, SHL, xxx, USC
    ...
    permissions = db.Column(
        db.Text
    )  # JSON: CAL_MEET_USER, GOD, RFPO_ADMIN, RFPO_USER, VROOM_ADMIN, VROOM_USER

    # Legacy fields (marked as deprecated in forms)
    global_admin = db.Column(
        db.Boolean, default=False
    )  # Deprecated: "won't be used in future"
    use_rfpo = db.Column(
        db.Boolean, default=False
    )  # Deprecated: "won't be used in future"
    ...
    max_upload_size = db.Column(
        db.Integer, default=8388608
    )  # Max upload allowed (in bytes)
    ...
    is_approver = db.Column(
        db.Boolean, default=False
    )  # True if user is assigned to any approval workflow
    approver_updated_at = db.Column(
        db.DateTime
    ...
    def add_to_team(self, team_id: int, role: str = "member", created_by: Optional[str] = None) -> None:
        ...
        if not self.is_member_of_team(team_id):
            user_team = UserTeam(
                user_id=self.id, team_id=team_id, role=role, created_by=created_by
            )
    ...
    def check_approver_status(self):
        ...
        primary_steps = RFPOApprovalStep.query.filter_by(
            primary_approver_id=self.record_id
        ).all()
        backup_steps = RFPOApprovalStep.query.filter_by(
            backup_approver_id=self.record_id
        ).all()
        ...
        for step in all_steps:
            if step.stage and step.stage.workflow:
                ...
                workflow_info = {
                    ...
                    "role": (
                        "primary"
                        if step.primary_approver_id == self.record_id
                        else "backup"
                    ),
                }
        ...
        valid_primary_count = sum(
            1 for step in primary_steps if step.stage and step.stage.workflow
        )
        valid_backup_count = sum(
            1 for step in backup_steps if step.stage and step.stage.workflow
        )
    ...
    def get_approver_summary(self):
        ...
        return {
            ...
            "last_updated": (
                self.approver_updated_at.isoformat()
                if self.approver_updated_at
                else None
            ),
...
class UserTeam(db.Model):
    ...
    user = db.relationship(
        "User",
        backref=db.backref("user_teams", lazy=True, cascade="all, delete-orphan"),
    )
    team = db.relationship(
        "Team",
        backref=db.backref("team_users", lazy=True, cascade="all, delete-orphan"),
    )
...
class Project(db.Model):
    ...
    project_id = db.Column(
        db.String(32), unique=True, nullable=False
    )  # External project ID (e.g., "00000317")
    ...
    consortium_ids = db.Column(
        db.Text
    )  # project_consort[] - JSON array of consortium consort_ids

    # Optional team association
    team_record_id = db.Column(
        db.String(32), nullable=True
    )  # project_team (team's record_id)

    # Project member permissions
    rfpo_viewer_user_ids = db.Column(
        db.Text
    )  # project_rfpo_viewers[] - JSON array of user IDs

    # Project classification flags
    gov_funded = db.Column(
        db.Boolean, default=True
    )  # project_gov (Gov Funded checkbox)
    uni_project = db.Column(
        db.Boolean, default=False
    )  # project_uni (Uni Project checkbox)
    ...
    def __repr__(self):
        consortium_info = (
            f" ({len(self.get_consortium_ids())} consortiums)"
            if self.get_consortium_ids()
            else " (No consortium)"
        )
...
class Vendor(db.Model):
    ...
    vendor_id = db.Column(
        db.String(32), unique=True, nullable=False
    )  # External vendor ID (e.g., "00000865")
    ...
    certs_reps = db.Column(
        db.Boolean, default=False
    )  # vendor_certs (Certs & Reps checkbox)
    cert_date = db.Column(db.Date)  # vendor_cert_date (certification creation date)
    cert_expire_date = db.Column(
        db.Date
    )  # vendor_cert_expire_date (certification expiration)
    is_university = db.Column(
        db.Boolean, default=False
    )  # vendor_uni (Uni checkbox - disabled in form)
    vendor_type = db.Column(
        db.Integer, default=0
    )  # vendor_type2 (0=None, 1=University, 2=Small Business, 3=Non Profit)

    # Consortium Approvals (stored as JSON for the checkbox fields)
    approved_consortiums = db.Column(
        db.Text
    )  # consort_0 through consort_9 checkboxes as JSON array

    # One-time vendor project association
    onetime_project_id = db.Column(
        db.String(32), nullable=True
    )  # vendor_onetime (project_id)
    ...
    def get_vendor_type_display(self):
        ...
        type_map = {0: "None", 1: "University", 2: "Small Business", 3: "Non Profit"}
    ...
    def to_dict(self):
        return {
            ...
            "cert_expire_date": (
                self.cert_expire_date.isoformat() if self.cert_expire_date else None
            ),
    ...
    def __repr__(self):
        ...
        return (
            f"<Vendor {self.vendor_id}: {self.company_name}{status_info}{onetime_info}>"
        )
...
class VendorSite(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    vendor_site_id = db.Column(
        db.String(32), unique=True, nullable=False
    )  # External site ID (e.g., "00000905")
    ...
    vendor = db.relationship(
        "Vendor", backref=db.backref("sites", lazy=True, cascade="all, delete-orphan")
    )
    ...
    def __repr__(self):
        ...
        return (
            f"<VendorSite {self.vendor_site_id}: {self.contact_name} @ {vendor_name}>"
        )
...
class PDFPositioning(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    consortium_id = db.Column(
        db.String(32), nullable=False
    )  # e.g., "00000014" for USCAR
    ...
    def set_field_position(
        self, field_name, x, y, font_size=9, font_weight="normal", visible=True
    ):
...
class List(db.Model):
    ...
    list_id = db.Column(
        db.String(32), unique=True, nullable=False
    )  # External list ID (e.g., "0000000021")
    ...
    __table_args__ = (db.UniqueConstraint("type", "key", name="uq_list_type_key"),)
    ...
    def get_by_type_ci(cls, list_type):
        ...
        return (
            cls.query.filter(db.func.lower(cls.type) == list_type.lower())
    ...
    def get_item_ci(cls, list_type, key):
        ...
        return (
            cls.query.filter(
                db.func.lower(cls.type) == list_type.lower(), cls.key == key
            )
            .filter((cls.active == True) | (cls.active.is_(None)))  # noqa: E712
            .first()
        )
...
class RFPOApprovalWorkflow(db.Model):
    ...
    name = db.Column(
        db.String(255), nullable=False
    )  # Workflow name (e.g., "USCAR Standard Approval")
    ...
    workflow_type = db.Column(
        db.String(20), nullable=False, default="consortium"
    )  # 'consortium', 'team', 'project'
    consortium_id = db.Column(
        db.String(32), nullable=True
    )  # Consortium consort_id (nullable for team/project workflows)
    team_id = db.Column(
        db.Integer, db.ForeignKey("teams.id"), nullable=True
    )  # Team ID for team workflows
    project_id = db.Column(
        db.String(32), nullable=True
    )  # Project ID for project workflows
    ...
    is_template = db.Column(
        db.Boolean, default=True
    )  # True for templates, False for instances
    ...
    stages = db.relationship(
        ...
        order_by="RFPOApprovalStage.stage_order",
    )
    instances = db.relationship(
        "RFPOApprovalInstance", backref="template_workflow", lazy=True
    )
    ...
    __table_args__ = (
        db.Index(
            "idx_consortium_type_active", "consortium_id", "workflow_type", "is_active"
        ),
    ...
    def activate(self):
        ...
        if self.workflow_type == "consortium":
            ...
            db.session.query(RFPOApprovalWorkflow).filter_by(
                team_id=self.team_id, workflow_type="team", is_template=True
            ).update({"is_active": False})
...
class RFPOApprovalStage(db.Model):
    ...
    stage_name = db.Column(
        db.String(255), nullable=False
    )  # Stage name (e.g., "Under $5,000")
    stage_order = db.Column(
        db.Integer, nullable=False
    )  # Order of this stage in workflow (1, 2, 3...)
    description = db.Column(db.Text)  # Stage description

    # Budget Bracket Association
    budget_bracket_key = db.Column(
        db.String(255), nullable=False
    )  # References RFPO_BRACK key from Lists
    budget_bracket_amount = db.Column(
        db.Numeric(12, 2), nullable=False
    )  # Cached amount for performance

    # Workflow Association
    workflow_id = db.Column(
        db.Integer, db.ForeignKey("rfpo_approval_workflows.id"), nullable=False
    )

    # Stage Configuration
    requires_all_steps = db.Column(
        db.Boolean, default=True
    )  # True: all steps must approve, False: any step can approve
    is_parallel = db.Column(
        db.Boolean, default=False
    )  # True: steps can approve in parallel, False: sequential

    # Required Document Types (stored as JSON array of doc_types keys)
    required_document_types = db.Column(
        db.Text
    )  # JSON array of document type keys from doc_types list
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "workflow_id", "budget_bracket_key", name="uq_workflow_bracket"
        ),
        db.UniqueConstraint(
            "workflow_id", "stage_order", name="uq_workflow_stage_order"
        ),
    )
    ...
    def get_required_approvers(self):
        ...
        return [
            step.primary_approver_id for step in self.steps if step.primary_approver_id
        ]

    def get_backup_approvers(self):
        """Get list of backup approvers for this stage"""
        return [
            step.backup_approver_id for step in self.steps if step.backup_approver_id
        ]
    ...
    def to_dict(self):
        return {
            ...
            "budget_bracket_amount": (
                float(self.budget_bracket_amount)
                if self.budget_bracket_amount
                else 0.00
            ),
            "workflow_id": self.workflow_id,
...
class RFPOApprovalStep(db.Model):
    ...
    step_name = db.Column(
        db.String(255), nullable=False
    )  # Step name (e.g., "Technical Review")
    step_order = db.Column(
        db.Integer, nullable=False
    )  # Order of this step in stage (1, 2, 3...)
    description = db.Column(db.Text)  # Step description

    # RFPO_APPRO Association
    approval_type_key = db.Column(
        db.String(255), nullable=False
    )  # References RFPO_APPRO key from Lists
    approval_type_name = db.Column(
        db.String(255), nullable=False
    )  # Cached name for performance

    # Stage Association
    stage_id = db.Column(
        db.Integer, db.ForeignKey("rfpo_approval_stages.id"), nullable=False
    )

    # Approver Configuration
    primary_approver_id = db.Column(
        db.String(32), nullable=False
    )  # User record_id of primary approver
    backup_approver_id = db.Column(
        db.String(32), nullable=True
    )  # User record_id of backup approver (optional)

    # Step Configuration
    is_required = db.Column(
        db.Boolean, default=True
    )  # True: step must be completed, False: optional
    timeout_days = db.Column(db.Integer, default=5)  # Days before step times out
    auto_escalate = db.Column(
        db.Boolean, default=False
    )  # Auto-escalate to backup after timeout
    ...
    def get_primary_approver(self):
        ...
        return User.query.filter_by(
            record_id=self.primary_approver_id, active=True
        ).first()
    ...
    def get_backup_approver(self):
        ...
        if self.backup_approver_id:
            return User.query.filter_by(
                record_id=self.backup_approver_id, active=True
            ).first()
        return None
    ...
    def to_dict(self):
        ...
        return {
            ...
            "primary_approver_name": (
                primary_approver.get_display_name() if primary_approver else None
            ),
            "backup_approver_id": self.backup_approver_id,
            "backup_approver_name": (
                backup_approver.get_display_name() if backup_approver else None
            ),
...
class RFPOApprovalInstance(db.Model):
    ...
    template_workflow_id = db.Column(
        db.Integer, db.ForeignKey("rfpo_approval_workflows.id"), nullable=False
    )
    ...
    workflow_name = db.Column(
        db.String(255), nullable=False
    )  # Snapshot of workflow name
    workflow_version = db.Column(
        db.String(20), nullable=False
    )  # Snapshot of workflow version
    consortium_id = db.Column(
        db.String(32), nullable=False
    )  # Snapshot of consortium ID

    # Instance Status
    current_stage_order = db.Column(
        db.Integer, default=1
    )  # Current stage being processed
    current_step_order = db.Column(
        db.Integer, default=1
    ...
    instance_data = db.Column(
        db.Text
    )  # JSON snapshot of workflow configuration at creation time
    ...
    actions = db.relationship(
        ...
        lazy=True,
        cascade="all, delete-orphan",
        order_by="RFPOApprovalAction.created_at",
    )
...
class RFPOApprovalAction(db.Model):
    ...
    instance_id = db.Column(
        db.Integer, db.ForeignKey("rfpo_approval_instances.id"), nullable=False
    )

    # Action Context (snapshot from instance at time of action)
    stage_order = db.Column(
        db.Integer, nullable=False
    )  # Which stage this action is for
    ...
    approval_type_key = db.Column(
        db.String(255), nullable=False
    )  # Snapshot of approval type

    # Approver Information
    approver_id = db.Column(
        db.String(32), nullable=False
    )  # User record_id who took this action
    approver_name = db.Column(
        db.String(255), nullable=False
    )  # Snapshot of approver name
    is_backup_approver = db.Column(
        db.Boolean, default=False
    )  # True if backup approver took action

    # Action Details
    status = db.Column(
        db.String(32), nullable=False
    )  # Uses RFPO_STATU values: approved, conditional, refused, pending
    comments = db.Column(db.Text)  # Approver comments
    conditions = db.Column(
        db.Text
    )  # Conditional approval conditions (if status = conditional)

    # Timing
    assigned_at = db.Column(
        db.DateTime, default=datetime.utcnow
    )  # When action was assigned
...
class Ticket(db.Model):
    ...
    severity = db.Column(db.String(20), nullable=True)  # bug only
    page_url = db.Column(db.String(512), nullable=True)
    browser_info = db.Column(db.String(512), nullable=True)
    ...
    __table_args__ = (
        ...
        db.Index("idx_ticket_created_by_type", "created_by", "type"),
    )

    comments = db.relationship(
        "TicketComment", backref="ticket", cascade="all, delete-orphan",
        order_by="TicketComment.created_at", lazy=True,
    )
    attachments = db.relationship(
        "TicketAttachment", backref="ticket", cascade="all, delete-orphan", lazy=True,
    )
    creator = db.relationship("User", foreign_keys=[created_by], backref="tickets_created")
    assignee = db.relationship("User", foreign_keys=[assigned_to], backref="tickets_assigned")

    @staticmethod
    def generate_ticket_number(ticket_type, session):
        """Generate next sequential ticket number: BUG-0001 or FR-0001."""
        prefix = "BUG" if ticket_type == "bug" else "FR"
        last = (
            session.query(Ticket)
            .filter(Ticket.type == ticket_type)
    ...
    def to_dict_public(self):
        """Serialization for non-admin users — hides internal_notes."""
        d = self.to_dict()
        d.pop("internal_notes", None)
        # Only count public comments
        d["comment_count"] = sum(1 for c in (self.comments or []) if not c.is_internal)
        return d
...
class TicketAttachment(db.Model):
    ...
    file_path = db.Column(db.String(512), nullable=False)
    cloud_path = db.Column(db.String(512), nullable=True)
