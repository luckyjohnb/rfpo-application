...
def test_consortium_improvements():
    ...
    if response.status_code == 200:
        ...
        if (
            ...
            print(
                "✅ Available/Selected user interface present for both viewers and admins"
            )
        else:
        ...
        if (
            "invoicing_street" in response.text
            and "invoicing_city" in response.text
            and "invoicing_state" in response.text
        ):
        ...
        if (
            "Non-Government Project" in response.text
