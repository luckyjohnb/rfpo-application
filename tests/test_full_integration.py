...
class FullIntegrationTester:
        ...
        for url, name in form_tests:
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"❌ {name} create form failed - Status: {response.status_code}"
                    )
                    return False
            except Exception as e:
    ...
    def test_rfpo_full_crud(self):
        ...
        try:
            # CREATE
            response = self.session.post(
                f"{self.base_url}/rfpo/new", data=test_data, allow_redirects=False
            )
            if response.status_code == 302:
    ...
    def test_project_full_crud(self):
        ...
        try:
            # CREATE
            response = self.session.post(
                f"{self.base_url}/project/new", data=test_data, allow_redirects=False
            )
            if response.status_code == 302:
    ...
    def test_vendor_full_crud(self):
        ...
        try:
            # CREATE
            response = self.session.post(
                f"{self.base_url}/vendor/new", data=test_data, allow_redirects=False
            )
            if response.status_code == 302:
    ...
    def test_user_full_crud(self):
        ...
        try:
            # CREATE
            response = self.session.post(
                f"{self.base_url}/user/new", data=test_data, allow_redirects=False
            )
            if response.status_code == 302:
    ...
    def test_json_field_integrity(self):
        ...
        try:
            ...
            response = self.session.get(f"{self.base_url}/consortiums")
            if (
                "viewer1, viewer2, viewer3" in response.text
                or "admin1, admin2" in response.text
            ):
