...
def test_rag_search(rfpo):
    ...
    with app.app_context():
        ...
        for query in test_queries:
            ...
            try:
                results = document_processor.search_similar_chunks(query, rfpo.id, top_k=3)
...
def test_ai_integration(rfpo):
    ...
    with app.app_context():
        ...
        try:
            ...
            test_message = "Can you summarize the project requirements and vendor recommendations?"
            user_context = {'current_rfpo_id': rfpo.id}
            
            print(f"💬 Test message: {test_message}")
            
            enhanced_data = rag_assistant.enhance_message_with_rag(test_message, user_context)
            
            print(f"🔧 Context source: {enhanced_data.get('context_source', 'unknown')}")
...
def cleanup_test_data():
    ...
    with app.app_context():
        try:
            ...
            # Delete files
            files_deleted = UploadedFile.query.filter_by(uploaded_by='test_user').delete()
