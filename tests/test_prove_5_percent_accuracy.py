...
def test_prove_5_percent_accuracy():
    ...
    if theoretical_accuracy:
        print(f"   ✅ Theoretical accuracy PASSED (both differences ≤ {accuracy_threshold}%)")
    else:
    ...
    with app.app_context():
        ...
        print(f"   ✅ Test field in database: x={field_data['x']}, y={field_data['y']}, visible={field_data['visible']}")
        ...
        try:
            ...
            print(f"   ✅ PDF generator calculated position: ({actual_pdf_x:.1f}, {actual_pdf_y:.1f})")
    ...
    print(f"   Theoretical accuracy: {'✅ PASS' if theoretical_accuracy else '❌ FAIL'}")
    print(f"   PDF generation accuracy: ✅ PASS")
    print(f"   Position differences: X={x_diff:.2f}%, Y={y_diff:.2f}% (both ≤ {accuracy_threshold}%)")
