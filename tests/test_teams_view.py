...
def test_teams_view():
    ...
    if response.status_code == 200:
        ...
        if (
            'data-bs-toggle="tooltip"' in response.text
            and "consortium-badge" in response.text
        ):
        ...
        # Check for team data
        if (
            "Research & Development Team" in response.text
            or "Test Team" in response.text
        ):
    ...
    print(
        "   • Proper column ordering: ID, Name, Abbrev, Consortium, Description, Projects, Viewers, Admins, Status"
    )
