...
def create_team():
    ...
    try:
        ...
        if not data.get('name') or not data.get('abbrev') or not data.get('consortium_id'):
            return jsonify({'success': False, 'message': 'Name, abbreviation, and consortium are required'}), 400
        ...
        db.session.rollback()
        return jsonify({'success': False, 'message': 'Team name or abbreviation already exists in this consortium'}), 400
    except Exception as e:
        db.session.rollback()
        return jsonify({'success': False, 'message': str(e)}), 500
...
def update_team(team_id):
    ...
    try:
        ...
        team.limited_admin_user_ids = data.get('limited_admin_user_ids', team.limited_admin_user_ids)
...
def delete_team(team_id):
    ...
    try:
        ...
        if 'Administrator' not in request.current_user.roles:
            return jsonify({'success': False, 'message': 'System administrator access required'}), 403
